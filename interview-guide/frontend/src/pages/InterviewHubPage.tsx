...
import {
  ChevronDown, ChevronUp, FileStack, FileText, Loader2, Mic, Bot,
  RefreshCw, Sparkles,
} from 'lucide-react';
...
export default function InterviewHubPage() {
  ...
  const handleStart = () => {
    ...
    } else if (config.mode === 'omni-voice') {
      const params = new URLSearchParams({
        skillId: config.skillId,
        difficulty: config.difficulty,
      });
      if (config.resumeId) params.set('resumeId', String(config.resumeId));
      navigate(`/omni-voice?${params.toString()}`);
    } else {
    ...
    <div className="max-w-5xl mx-auto">
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 p-6 mb-8">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {([
                ...
                {
                  value: 'omni-voice' as InterviewMode,
                  label: '语音对话 (Omni)',
                  icon: Bot,
                  desc: '可打断实时对话，原生语音交互',
                  recommended: false,
          ...
          <AnimatePresence>
            {config.showMore && (
              ...
              >
                ...
                {/* 语音面试 - 时长 */}
                {(config.mode === 'voice' || config.mode === 'omni-voice') && (
        ...
        <div className="mt-6 pt-6 border-t border-slate-100 dark:border-slate-700">
          ...
          >
            开始{config.mode === 'text' ? '文字' : config.mode === 'omni-voice' ? '语音对话 (Omni)' : '语音'}面试
          </motion.button>
        </div>
      </div>
