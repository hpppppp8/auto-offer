...
import type {
  ...
  ProviderTestResult, AsrConfig, TtsConfig, OmniConfig,
  AsrConfigRequest, TtsConfigRequest, OmniConfigRequest,
...
const PROVIDER_PRESETS: Record<string, {
  ...
  dashscope: {
    ...
    models: [
      ...
      { value: 'qwen3.5-omni-flash-realtime', label: 'Qwen-Omni-Realtime — 语音对话' },
...
export default function SettingsPage() {
  ...
  const [omniConfig, setOmniConfig] = useState<OmniConfig | null>(null);
  const [showVoiceModal, setShowVoiceModal] = useState<'asr' | 'tts' | 'omni' | null>(null);
  const [testingAsr, setTestingAsr] = useState(false);
  const [asrTestResult, setAsrTestResult] = useState<ProviderTestResult | null>(null);
  const [voiceSaving, setVoiceSaving] = useState(false);

  // ASR/TTS/Omni form fields
  const [asrForm, setAsrForm] = useState<AsrConfigRequest>({});
  const [ttsForm, setTtsForm] = useState<TtsConfigRequest>({});
  const [omniForm, setOmniForm] = useState<OmniConfigRequest>({});
  const [testingOmni, setTestingOmni] = useState(false);
  const [omniTestResult, setOmniTestResult] = useState<ProviderTestResult | null>(null);
  ...
  const loadData = useCallback(async () => {
    try {
      const [providerList, defaultProvider, asr, tts, omni] = await Promise.all([
        llmProviderApi.list(),
        llmProviderApi.getDefaultProvider(),
        llmProviderApi.getAsrConfig(),
        llmProviderApi.getTtsConfig(),
        llmProviderApi.getOmniConfig(),
      ]);
  ...
  const openOmniModal = () => {
    ...
    setOmniForm({
      ...
      voice: omniConfig.voice,
      silenceDurationMs: omniConfig.silenceDurationMs,
    });
    setShowVoiceModal('omni');
  };

  const handleSaveOmni = async () => {
    setVoiceSaving(true);
    try {
      await llmProviderApi.updateOmniConfig(omniForm);
      showToast('Omni 配置已更新');
      setShowVoiceModal(null);
      await loadData();
    } catch (err) {
      showToast(err instanceof Error ? err.message : '更新失败', 'error');
  ...
  const handleTestOmni = async () => {
    setTestingOmni(true);
    setOmniTestResult(null);
    try {
      const result = await llmProviderApi.testOmni();
                      ...
                      <div className="mb-4 flex items-start justify-between gap-3">
                        <div className="flex min-w-0 items-center gap-3">
                          ...
                          <div className="min-w-0">
                            ...
                            <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">统一实时语音对话 (ASR+LLM+TTS)</p>
                          </div>
                        </div>
                        <StatusBadge icon={<Mic className="h-3 w-3" />}>可打断</StatusBadge>
                      </div>

                      <dl className={DETAILS_CLASS}>
                        <ConfigRow label="模型" value={omniConfig.model} title={omniConfig.model} emphasis />
                        <ConfigRow label="音色" value={omniConfig.voice} title={omniConfig.voice} emphasis />
                        <ConfigRow label="静默检测" value={`${omniConfig.silenceDurationMs}ms`} />
                        <ConfigRow
                          label="API Key"
                          value={omniConfig.maskedApiKey}
                          title={omniConfig.maskedApiKey}
                          monospace
                          emphasis
                        />
                      </dl>
                      ...
                      {omniTestResult && (
                        <div className={`mb-3 px-3 py-2 rounded-lg text-xs font-medium ${
                          ...
                          <div className="flex items-center gap-1.5">
                            ...
                            <span>{omniTestResult.message}</span>
                          </div>
                        </div>
                      )}
        ...
        {showVoiceModal && (
          <>
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
              ...
              >
                <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-5">
                  {showVoiceModal === 'asr' ? '编辑 ASR 语音识别' : showVoiceModal === 'tts' ? '编辑 TTS 语音合成' : '编辑 Qwen-Omni 实时语音'}
                </h3>
                ...
                {showVoiceModal === 'asr' ? (
                  ...
                  <div className="space-y-4">
                    <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">模型配置</p>
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">Model</label>
                        <input type="text" value={omniForm.model || ''} onChange={(e) => setOmniForm(f => ({ ...f, model: e.target.value }))}
                          className="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-sm text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary-500/50 focus:border-primary-400 transition-shadow" />
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">Voice</label>
                        <input type="text" value={omniForm.voice || ''} onChange={(e) => setOmniForm(f => ({ ...f, voice: e.target.value }))}
                          className="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-sm text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary-500/50 focus:border-primary-400 transition-shadow" />
                      </div>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">Silence Duration (ms)</label>
                      <input type="number" value={omniForm.silenceDurationMs || 0} onChange={(e) => setOmniForm(f => ({ ...f, silenceDurationMs: Number(e.target.value) }))}
                        className="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-sm text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary-500/50 focus:border-primary-400 transition-shadow" />
                    </div>

                    <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider pt-2">认证</p>
                    <div>
                      <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">API Key <span className="text-slate-400 font-normal">(留空不改)</span></label>
                      <input type="password" value={omniForm.apiKey || ''} onChange={(e) => setOmniForm(f => ({ ...f, apiKey: e.target.value }))}
                        placeholder="留空则保持原值"
                        className="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-sm text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary-500/50 focus:border-primary-400 transition-shadow" />
                    </div>
                  </div>
                ...
                <div className="flex gap-3 justify-end mt-6">
                  ...
                  <motion.button
                    onClick={showVoiceModal === 'asr' ? handleSaveAsr : showVoiceModal === 'tts' ? handleSaveTts : handleSaveOmni}
                    disabled={voiceSaving}
